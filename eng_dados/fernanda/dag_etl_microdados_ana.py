...
from funcoes_ana import extract_all, Dataset, parse
import os
...
dag = DAG(
    dag_id='etl_microdados_inep_ana',
...
scrape_data = BashOperator(
    task_id='scrape_data',
    bash_command=f'cd {SCRAPERS_HOME} && scrapy crawl microdados_ana -a data_path={TMP_DATA}',
    dag=dag
)
...
parse_data = PythonOperator(
    ...
    op_kwargs={
        'path': os.path.join(DATA, 'dados_2014.csv'),
        'catalog': os.path.join(METADATA, 'dicionario_dados_2014.xlsx#TS_ESCOLA'),
        'year': 2014,
        'dest': DATA
    },
    dag=dag
